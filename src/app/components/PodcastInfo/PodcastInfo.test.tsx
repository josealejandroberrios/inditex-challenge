...
import PodcastInfo from './index';
import { PodcastInfoProps } from './types';

const defaultProps: PodcastInfoProps = {
  id: '1685691481',
  title: 'Scamanda',
...
describe('<PodcastInfo />', () => {
  test('should render with props', () => {
    wrapperSetup(<PodcastInfo {...defaultProps} />);
    const image = screen.getByTestId('PodcastInfo-image') as HTMLImageElement;
    expect(image).toBeDefined();
    expect(image.src).toBe(defaultProps.imageUrl);

    const title = screen.getByTestId('PodcastInfo-title');
    expect(title).toBeDefined();
    expect(title.textContent).toBe(defaultProps.title);

    const author = screen.getByTestId('PodcastInfo-author');
    expect(author).toBeDefined();
    expect(author.textContent).toBe(`by ${defaultProps.author}`);

    const description = screen.getByTestId('PodcastInfo-description');
